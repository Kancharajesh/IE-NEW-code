...
public class Post_login extends Basetest {
	...
	public void Login_Dashboard() throws InterruptedException {
		ExtentTest test = extent.createTest("Login Dashboard Test");
		login();
		Thread.sleep(2500);

		test.log(Status.INFO, "Verifying Dashboard main screen");
		...
		test.log(Status.PASS, "Dashboard screen text matches the expected text");
	...
	public void Overview() throws InterruptedException {
	    ...
	    // Sleep for a few seconds to ensure the page loads completely
	    Thread.sleep(3000);

	    // Find the overview element
	    WebElement overviewElement = driver.findElement(By.xpath(prop.getProperty("Overview")));

	    // Get the text of the overview element
	    String overviewText = overviewElement.getText();

	    if (overviewText.isEmpty()) {
	        System.out.println("Overview text is empty");
	        Assert.fail("Overview text is empty");
	    } else {
	        // Print overview text
	...
	public void print_the_firstsurvey_response() throws InterruptedException {
	    ...
	    // Scroll down the page to bring survey cards into view
	    JavascriptExecutor js = (JavascriptExecutor) driver;
	    js.executeScript("window.scrollBy(250,350)", "");
	    Thread.sleep(1500);

	    // Verify the presence of the survey card element before clicking
	    WebElement surveyCard = driver.findElement(By.xpath(prop.getProperty("surveycards")));
	    Assert.assertTrue(surveyCard.isDisplayed(), "Survey card is not displayed");
	    surveyCard.click();
	    Thread.sleep(2000);

	    // Scroll down to bring the survey questions into view
	    js.executeScript("window.scrollBy(250,1050)", "");
	    Thread.sleep(3500);

	    // Verify the presence of survey question elements before printing them
	    List<WebElement> surveyQuestions = driver.findElements(By.xpath(prop.getProperty("surveycardquestion")));
	    Assert.assertFalse(surveyQuestions.isEmpty(), "No survey questions found");

	    // Print the text of each survey question
	    for (WebElement question : surveyQuestions) {
	        System.out.println("Survey Question: " + question.getText());
	    }
	}
